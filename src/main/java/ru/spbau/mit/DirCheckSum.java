...
public class DirCheckSum {
    ...
    public static String checkSumOneThread(Path path) throws NoSuchAlgorithmException, FileNotFoundException {
        MessageDigest md5 = MessageDigest.getInstance("MD5");

        if(Files.isDirectory(path)) {
            md5.update(path.toString().getBytes());
            for(File fileEntry: path.toFile().listFiles()) {
                md5.update(checkSumOneThread(fileEntry.toPath()).getBytes());
            }
        } else {
